...
class TestcoSum:
    ...
    cosum = coSum()
    solver = Permutation(
        coSum.obj_func, cosum.candidates, cosum.constraint, metrics="l2"
    )
    solution = solver.minimize(
        max_iter=5, selection_size=cosum.selection_size, population=20, seed=seed
    )
...
class TestTSP:
    ...
    solution = solver.minimize(max_iter=50, population=15, seed=seed)
